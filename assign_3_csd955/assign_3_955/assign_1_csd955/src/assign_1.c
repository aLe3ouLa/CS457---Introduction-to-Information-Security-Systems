...
unsigned char * keygen(unsigned char *, unsigned char *, int *, unsigned char *, int);
void encrypt(unsigned char *, int, unsigned char *, unsigned char *, 
    unsigned char *, int );
int decrypt(unsigned char *, int, unsigned char *, unsigned char *, 
    unsigned char *, int);
void gen_cmac(unsigned char *, size_t, unsigned char *, int, unsigned char *, int);
 ...
 */
unsigned char *
keygen(unsigned char *password, unsigned char *key, int *keyLen, unsigned char *iv,
    int bit_mode)
{
 ...
 */
void
gen_cmac(unsigned char *data, size_t data_len, unsigned char *key, int key_len,
    unsigned char *cmac, int bit_mode)
{
        ...
        switch (bit_mode){
            case 128:
                if(CMAC_Init(ctx, key, key_len, EVP_aes_128_ecb(), NULL) != 1){
                    fprintf(stderr, "CMAC_Init failed\n");
                    return;
                }
                break;
            case 256:
                if(CMAC_Init(ctx, key, key_len, EVP_aes_256_ecb(), NULL) != 1){
                    fprintf(stderr, "CMAC_Init failed\n");
                    return;
                }
                break;
...
main(int argc, char **argv)
{
	...
	/* Keygen from password */
        myKey = keygen(password, myKey, &keyLength, myIV , bit_mode);
        ...
        switch (op_mode){
            ...
            case 2:
                ...
                gen_cmac(plaintext, lSize, myKey, keyLength, cmac, bit_mode);
                printf("cmac: \n");
                print_hex(cmac, 16);
            ...
            case 3:
                ...
                /* Generate cmac for plaintext */
                cmac = malloc (BLOCK_SIZE * sizeof(char));
                gen_cmac(plaintext, plaintext_length, myKey, keyLength, cmac, bit_mode);
